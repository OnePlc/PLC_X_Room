<?php
/**
 * ApiController.php - Room Api Controller
 *
 * Main Controller for Room Api
 *
 * @category Controller
 * @package Application
...
use OnePlace\Room\Model\FurnitureTable;
use OnePlace\Room\Model\RoomTable;
use Laminas\View\Model\ViewModel;
use Laminas\Db\Adapter\AdapterInterface;
use Zend\I18n\Translator\Translator;

class FurnitureApiController extends CoreController {
    /**
     * Room Table Object
     *
     * @since 1.0.0
     ...
     * @since 1.0.0
     */
    public function __construct(AdapterInterface $oDbAdapter,FurnitureTable $oTableGateway,$oServiceManager) {
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Room API'];
    ...
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('room')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for room found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'room', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('roomfurniture-single');
        $aFieldsByKey = [];
        ...
        # Get All Room Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'room',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'room',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'room',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'room',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    public function getAction() {
        ...
        try {
            ...
            $aReturn = ['state'=>'error','message'=>'Room not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Room found','oItem'=>$oItem];
        echo json_encode($aReturn);
